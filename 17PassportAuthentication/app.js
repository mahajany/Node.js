...
const URL = 'mongodb://192.168.2.50:27017/ognom';
...
function auth(req, res, next){
  ...
  console.log("AUTH FUNCTION:" + JSON.stringify(req.headers));
  
  //if(!req.signedCookies){
    //if(!req.session.user){    
  
    //if(!req.session.passport.user){          ///passport adds req.user
    if(!req.user){          ///passport adds req.user
